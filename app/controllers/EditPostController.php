...
class EditPostController extends PageController {
	...
	public function __construct($dbc) {
		...
		// Witout it you won't have plates!
		parent::__construct();
		...
		// Edit post is the name of the submit button
	...
	private function processPostEdit() {
		...
		if( strlen($title) > 100 ){
			$totalErrors++;
			$this->data['titleError'] = 'At most 100 characters';
		}

		if( strlen($desc) > 1000 ){
			$totalErrors++;
			$this->data['descError'] = 'At most 1000 characters';
		}
		...
		if( $totalErrors == 0 ) {
			...
			$postID = $this->dbc->real_escape_string($_GET['id']);
			// Delete the old images they are waisting space otherwise
